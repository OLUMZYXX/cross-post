  ...
  return (
    <div className="bg-neutral-900 border border-neutral-800/50 rounded-2xl flex flex-col h-full overflow-hidden">
      <div className="p-5 border-b border-neutral-800/30 flex items-center justify-between">
        ...
        <span className="text-neutral-600 text-xs font-semibold">{posts.length} posts</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        {posts.length === 0 ? (
          <div className="text-center py-10">
            ...
            <p className="text-neutral-500 text-sm font-medium">No posts scheduled</p>
            <p className="text-neutral-600 text-xs mt-1">Select a day with posts or schedule a new one</p>
          </div>
        ) : (
          posts.map((post) => {
            ...
            return (
              <div key={post._id} className="bg-neutral-800/30 hover:bg-neutral-800/60 p-4 rounded-xl transition-all group">
                ...
                <div className="flex gap-3">
                  {post.media?.[0] && (
                    <img src={post.media[0]} alt="" className="w-14 h-14 rounded-lg object-cover flex-shrink-0" />
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-white line-clamp-1 mb-1">{post.caption || "No caption"}</p>
                ...
                <div className="mt-3 pt-2 border-t border-neutral-800/50 flex items-center justify-between">
                  ...
                  {post.media?.length > 0 && (
                    <span className="text-[9px] text-neutral-600 font-medium">{post.media.length} media</span>
                  )}
                </div>
              </div>
            );
