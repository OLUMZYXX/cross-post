...
import { CheckCheck, Trash2, Bell, BellOff, CheckCircle, XCircle, AlertCircle, Clock, Link2, Unlink } from "lucide-react";
  ...
  return (
    <div className="animate-fade-in max-w-2xl">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-white font-headline">Notifications</h1>
          {unreadCount > 0 && <p className="text-green-400 text-[11px]">{unreadCount} unread</p>}
        </div>
      ...
      {notifications.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-neutral-600 text-xs mt-1">Notifications will appear here</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className={`text-sm font-medium truncate ${n.read ? "text-neutral-400" : "text-white"}`}>{n.title}</p>
                    {!n.read && <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: config.color }} />}
                  </div>
                  <p className={`text-[12px] mt-0.5 ${n.read ? "text-neutral-600" : "text-neutral-400"}`}>{n.message}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] uppercase tracking-wider" style={{ color: config.color }}>{config.label}</span>
                    <span className="text-neutral-600 text-[10px]">{timeAgo(n.createdAt)}</span>
                  </div>
                </div>
                <button
